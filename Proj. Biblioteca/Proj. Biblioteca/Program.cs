...
namespace Proj.Biblioteca
{
    internal class Program
    {
        ...
        static void RegistrarEmprestimo(Livros biblioteca)
        {
            ...
            if (livroEncontrado != null)
            {
                if (livroEncontrado.qtdeDisponiveis() > 0 && (livroEncontrado.qtdeEmprestimos() < livroEncontrado.qtdeExemplares()))
                {
                    if (livroEncontrado.Exemplares.Last().emprestar())
                    {
                        Console.WriteLine("Exemplar de livro emprestado.");
                    }
                    else
                    {
                        Console.WriteLine("Não foi possível emprestar esse exemplar.");
                    }
                }
                else
                {
        ...
        static void RegistrarDevolucao(Livros biblioteca)
        {
            ...
            if (livroEncontrado != null)
            {
                ...
                {
                    if (livroEncontrado.Exemplares.Last().devolver())
                    {
                        Console.WriteLine("Exemplar de livro emprestado.");
                    }
                    else
                    {
                        Console.WriteLine("Não foi possível devolver esse exemplar.");
                    }
                }
                else
                {
